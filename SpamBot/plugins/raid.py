...
from SpamBot import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9, SUDO_USERS, OWNER_ID
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(Deadly) == 2:
            ...
            g = a.id
            if int(g) in Deadly:
                text = f"I can't raid on ChutyaBot's Owner"
                await e.reply(text)
            elif int(g) == OWNER_ID:
            ...
            if int(g) in DEADLYSPAM:
                text = f"I can't raid on ChutyaBot Owner"
                await e.reply(text)
            elif int(g) == OWNER_ID:
                text = f"This guy is a owner Of this Bots."
                await e.reply(text)
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in Deadly:
                text = f" can't raid on ChutyaBot Owner."
                await e.reply(text)
